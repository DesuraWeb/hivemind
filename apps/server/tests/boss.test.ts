...
import { AUTH_HEALTHCHECK_QUEUE, createBoss, startBoss } from '../src/jobs/boss'
...
test('demarre, cree les queues, planifie le cron du healthcheck, et s arrete proprement', async () => {
  const boss = createBoss(env)
  const mailer = createMailer(env)
  const adapter = createFakeAdapter()

  await startBoss(boss, { db, adapter, mailer, alertTo: 'alerts@exemple.test' })

  try {
    const queues = await boss.getQueues([RUN_STEP_QUEUE, AUTH_HEALTHCHECK_QUEUE])
    expect(queues.map((q) => q.name).sort()).toEqual(
      [AUTH_HEALTHCHECK_QUEUE, RUN_STEP_QUEUE].sort(),
    )
